...
@section('content')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Vehicle Versions</h1>
    </div>
    <form action="{{ route('versions.index') }}" method="get" class="my-4">
        <input type="text", name="search" placeholder="Type version name">
        <button>Search</button>
        <a type="button" href="{{ route('register.version') }}" class="btn btn-success float-end btn-sm">Add Version</a>
    </form>
        ...
        <tbody>
            @foreach ($results as $result)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('update.version', $result->id) }}" class="btn btn-light btn-sm">Edit</a>
                        <meta name="csrf-token" content="{{ csrf_token() }}">
                        <a onclick="deletePagingRegistry('{{ route('version.delete') }}', {{ $result->id }})"
